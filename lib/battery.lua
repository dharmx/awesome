...
local WidgetBase = require("wibox.widget.base")
...
function M.get_BAT0_device_path()
...
local function default_template() return WidgetBase.empty_widget() end
...
function M.new(args)
  ...
  local widget = WidgetBase.make_widget_from_value(args.widget_template)

  widget.device = args.use_display_device and UPower.Client():get_display_device() or M.get_device(args.device_path)
  ...
  if args.instant_update then GearsTime.delayed_call(widget.emit_signal, widget, "upower::update", widget.device) end
